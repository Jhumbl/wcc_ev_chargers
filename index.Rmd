...
lccp_pending <- nrow(dplyr::filter(full_points, Provider=="Ubitricity" & Status == "Pending Installation"))
lccp_existing <- nrow(dplyr::filter(full_points, Provider=="Ubitricity" & Status == "Installed"))

rapid_pending <- nrow(dplyr::filter(full_points, Provider=="ESB" & Status == "Pending Installation"))
rapid_existing <- nrow(dplyr::filter(full_points, Provider=="ESB" & Status == "Installed"))

ded_pending <- nrow(dplyr::filter(full_points, !Provider %in% c("ESB", "Ubitricity") & Status == "Pending Installation"))
ded_existing <- nrow(dplyr::filter(full_points, !Provider %in% c("ESB", "Ubitricity") & Status == "Installed"))
...
leaflet() %>%
  ...
  addLegendCustom(colors = c("green", "#7DBF7A", "blue", "purple", "cyan"), labels = c("Lamp Column Charge Points (5kW)", "Planned Lamp Column Charge Points", "Dedicated Fast Chargers (7-22kW)",  "Rapid Charge Points (50kW)", "Planned Rapid Charge Points"), sizes = c(15, 15, 20, 25, 25)) %>%
  addLegend(position="bottomright", labels = c("Target: 1000",
                                                   paste("Lamp Column Charge Points", lccp_pending), 
                                                   paste("Rapids: ", rapid_pending)), 
                                               values="d", colors = rep("black", 3), title="Pending Installation:") %>%
