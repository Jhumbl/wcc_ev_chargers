...
ubi <- ubi %>% 
  ...
  as.data.frame() %>%
  #dplyr::rename("Latitude"="easting", "Longitude"="northing") %>%
  dplyr::select(Latitude, Longitude, Provider)
...
future_lccp <- read_csv("/Users/jackhumble/Documents/WestminsterPosition/organised/analysis_requests/2020:08:18_SiemensDesktopReviewProgress/confirmed_rollout.csv")
...
plot_map <- function(ward){
  ...
  lccp_ward %>%
    ...
    addCircles(data=future_lccp_ward, lng=~lon, lat=~lat, stroke = FALSE, fillColor = "orange", fillOpacity = 1, label = ~str_to_title(Address), radius = 8, group = "2020 Roll Out - Lamp Column Charge Points") %>%
